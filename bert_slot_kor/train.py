...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train", "-t", help="Path to training set", type=str, required=True
    )
    parser.add_argument(
        "--val",
        "-v",
    ...
    parser.add_argument(
        "--epochs", "-e", help="Number of epochs", type=int, default=5, required=False
    )
    parser.add_argument(
        "--batch", "-bs", help="Batch size", type=int, default=64, required=False
    )
    ...
    model = BertSlotModel(
        slots_num, bert_model_hub_path, sess, num_bert_fine_tune_layers=10
    )
