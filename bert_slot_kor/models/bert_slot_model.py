...
class BertSlotModel:
    ...
    def build_model(self):
        ...
        slots_output = TimeDistributed(Dense(self.slots_num, activation="softmax"))(
    ...
    def fit(self, X, Y, validation_data=None, epochs=5, batch_size=64):
        ...
        history = self.model.fit(
            X, Y, validation_data=validation_data, epochs=epochs, batch_size=batch_size
        )
    ...
    def predict_slots(self, x, slots_to_array, remove_start_end=True):
        ...
        if remove_start_end:
            ...
            y_slots = np.array(
                [
                    [x for x in y_slots[i][1 : (len(slots[i]) + 1)]]
    ...
    def load(load_folder_path, sess):  # load for inference or model evaluation
        ...
        new_model = BertSlotModel(
            slots_num, bert_hub_path, sess, num_bert_fine_tune_layers, is_training=False
        )
