...
def build_module_fn(config_path, vocab_path, do_lower_case=True):
    def bert_module_fn(is_training):
        ...
        token_type = tf.placeholder(
            shape=[None, None], dtype=tf.int32, name="segment_ids"
        )
        ...
        config_file = tf.constant(
            value=config_path, dtype=tf.string, name="config_file"
        )
        ...
        output_info_map = {"vocab_file": vocab_file, "do_lower_case": lower_case}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir", "-i", help="Etri KorBERT dir", type=str, required=True
    )
    parser.add_argument(
        "--output_dir", "-o", help="output_dir", type=str, required=True
    )
    ...
    spec.export(output_dir, checkpoint_path="{}/model.ckpt-56000".format(MODEL_DIR))
    print("complete")
